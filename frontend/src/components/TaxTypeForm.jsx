// frontend/src/components/TaxTypeForm.jsx
...
function TaxTypeForm() {
    ...
    useEffect(() => {
        if (isEditing) {
            ...
            axios.get(`${API_BASE_URL}/tax-types/${typeId}`)
                .then(response => { setName(response.data.name); setLoading(false); })
                .catch(err => { console.error("Error fetching tax type:", err); setError('Failed to load tax type data.'); setLoading(false); });
    ...
    const handleSubmit = async (e) => {
        ...
        if (!typeData.name) { setError("Tax type name cannot be empty."); setLoading(false); return; }

        try {
            if (isEditing) { await axios.put(`${API_BASE_URL}/tax-types/${typeId}`, typeData); }
            else { await axios.post(`${API_BASE_URL}/tax-types`, typeData); }
            navigate('/tax-types');
        } catch (err) { console.error("Error saving tax type:", err); setError(err.response?.data?.message || `Failed to ${isEditing ? 'update' : 'create'} tax type.`); }
        finally { setLoading(false); }
    };

    if (loading && isEditing) return <p>Loading tax type details...</p>;

    return (
        <div>
            <h2>{isEditing ? 'Edit Tax Type' : 'Add New Tax Type'}</h2>
            ...
            <form onSubmit={handleSubmit}>
                ...
                <div style={styles.buttonGroup}>
                    ...
                    <button type="button" onClick={() => navigate('/tax-types')} style={styles.buttonSecondary} disabled={loading}> Cancel </button>
                </div>
            </form>
        </div>
    );
...
// Basic Inline Styles
const styles = { /* ... copy styles from previous forms or define new ... */
