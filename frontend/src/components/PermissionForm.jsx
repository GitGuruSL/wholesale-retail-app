...
function PermissionForm() {
  ...
  const validateForm = () => {
    ...
    if (
      formData.permission_category_id === '' ||
      formData.permission_category_id === undefined ||
      formData.permission_category_id === null
    ) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      navigate('/dashboard/permissions', {
        state: { message: `Permission ${isEditing ? 'updated' : 'created'} successfully!` }
      });
    } catch (err) {
      ...
      <Box component="form" onSubmit={handleSubmit}>
        <FormControl fullWidth sx={{ mb: 2 }}>
          <TextField
            ...
            helperText={
              formErrors.name ||
              (isEditing && 'The permission name (code) cannot be changed after creation.')
            }
          />
        </FormControl>
        <FormControl fullWidth sx={{ mb: 2 }}>
